              ...
              if(args.model === "dall-e-3") {
                  ...
                  await Promise.all(promises).then(responses => {
                      responses.forEach((response, i) => {
                          if(response) {
                              ...
                              contentReturn += `Image ${i + 1}: ${img.revised_prompt ? img.revised_prompt : ""}\nURL: ${img.url}\n`;
                          } else {
                            contentReturn = "Sorry, I can't generate any image"
                          }
                      });
                  }).catch(() => {
                        contentReturn `Sorry, I can't generate any image` 
                ...
                if(response) {
                    response.data.forEach((img, idx) => {
                        imgList.push(img.url)
                        contentReturn += `Image ${idx + 1}: ${img.revised_prompt ? img.revised_prompt: ""}\nURL: ${img.url}\n`
                    })
                }
                else {
                    contentReturn = "Sorry, I can't generate any image"
