...
const followUpImage = {
    execute: async ({args, conversation, dependencies, countSystem, prepareKey, resource }) => {
        ...
        if(image_list && image_list.length > 0) {
          ...
          image_list.forEach(img => content.push({
            ...
            contentReturn = "no found any image"

        }
        
        const callGpt = common.handleCallGPTCommon(dependencies)
        const gptData = {
        ...
        const { completion } = await callGpt.execute(gptData)
        
        contentReturn = completion.choices[0].message.content
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    "prompt": {
                        "type": "string",
                        "description": "what user request about the image",
                    },
                },
                required: ["image_list"]
            }
